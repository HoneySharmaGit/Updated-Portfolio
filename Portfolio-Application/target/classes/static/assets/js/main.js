...
tabs.forEach((tab) => {
  tab.addEventListener("click", (el) => {
	const targetId = el.target.getAttribute("data-target");
...
/*==================== ROLEMODEL ====================*/
let swiperRolemodel = new Swiper(".rolemodel__container", {
	loop: true,
	grabCurson: true,
	spaceBetween: 48,
...
document.getElementById("contactForm").addEventListener("submit", function(event) {
	...
	xhr.open("POST", "/portfolio/sendform", true);
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState === 4) {
			if (xhr.status === 200) {
				...
				document.getElementById("responseMessage").style.display = "block"; // Show the element
				setTimeout(function() {
					document.getElementById("responseMessage").style.display = "none"; // Hide the element after 5 seconds
				}, 5000);
			} else {
				document.getElementById("responseMessage").innerHTML = "Error occurred. Please try again.";
				document.getElementById("responseMessage").style.display = "block"; // Show the element
				setTimeout(function() {
					document.getElementById("responseMessage").style.display = "none"; // Hide the element after 5 seconds
				}, 5000);
...
document.getElementById("contactForm").addEventListener("submit", function(event) {
	...
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "/portfolio/sendform");
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.onload = function() {
		if (xhr.status === 200) {
			// Request succeeded
